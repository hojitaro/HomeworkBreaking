...
window.onload = () => {
  ...
  const sbj = url.searchParams.get("sbj");
  const canvas = document.getElementById("canvas");
  const scoreboard = document.getElementById("scoreboard");
  const startBtn = document.getElementById("start");
  const state = document.getElementById("state");
  const ctx = canvas.getContext("2d");
  $.getJSON(`./${sbj}/config.json`, config => {
    /* 変更箇所 */
    ...
    const defSpeed = config.def_speed; // ブロックが落ちる初期速度
    const speedRate = config.speed_rate; // 十字下キーを押したときの加速度
    // imgフォルダの中に使いたい画像を追加した後、img: にその画像のパス、
    // score: に崩したときに得られる得点、chance: にそのブロックが出現する確率
    const tileData = config.tile_data;
    // ブロックがtileDataで指定した確率に沿って出現するかどうか
    // falseの場合、tileTypeの数字に沿ったブロックが出現します
    // 例えば、tileTypeで1の時は、tileDataの1番目の5円が落ちてくるようになります
    const randomTile = config.random_tile;
    // ブロックの落ちてくる形を指定
    // 1以上の数字の場所が、ブロックが存在し、0の部分はブロックは存在しません
    const tileType = config.tile_type;
    /* ******************************************************** */
